...
const sourceText = [
  'index.html',
  ...getFiles(path.join(rootDir, 'js'), '.js').map(file => path.relative(rootDir, file))
]
  .map(file => path.join(rootDir, file))
  .filter(file => fs.existsSync(file))
...
cssFiles.forEach(file => {
  ...
  const result = removeUnusedLogBlocks(before)
...
console.log('\nRemove unused Logg CSS:\n')

if (report.length === 0) {
  console.log('No unused Logg CSS blocks found.')
  process.exit(0)
}
...
if (!isWriteMode) {
  ...
  console.log('\n  node scripts/remove-unused-log-css.mjs --write\n')
  process.exit(0)
}
...
function removeUnusedLogBlocks(content) {
  const blocks = extractTopLevelCssBlocks(content)
  const removableBlocks = blocks.filter(isRemovableLogBlock)
...
function isRemovableLogBlock(block) {
  const classes = getClassesFromSelector(block.selector)

  if (classes.length === 0) return false

  const hasLogClass = classes.some(className => {
    return logCssPatterns.some(pattern => pattern.test(className))
  })

  if (!hasLogClass) return false
  ...
  return classes.every(className => {
    if (protectedClasses.has(className)) return false
